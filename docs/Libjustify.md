...
Libjustify is a simple library that wraps the printf() family of functions and offers a new family: `cprintf()` that emulates their behavior while automatically formatting to create a table.
...
Libjustify offers 4 new [[#Functions]] that are used with the same syntax as their `printf()` equivalents. 
...
These the format strings and store them as ([[#**Atoms **]]). ==To finally print and free all the data you must call `cflush()`==. Format strings are tabulated and justified to the width of the longest conversion specifier/text in that row. 
...
#### **Atoms:** 
Atoms represent a segment of the format string (and after manipulation the output) and are created by [[#Parsing format strings]] . When atoms are created they come in 3 general varieties: 
...
| Dummies               | Empty Atoms that store only navigation pointers. [[#Dummy Rows. Creating dummies & inserting the first atom]]

All Atoms are also nodes for a 2D doubly linked list  [[#Structure|Data Structure]]


## Structure

Atoms are nodes within a 2D linked list like graph. 
This 2D linked list contains a top and bottom row of dummies which are accessible through navigation or through `dummy_rows` a [[#`dummy_rows_ds`]] structure that holds helpful pointers to the corners of the dummy rows / graph
*Example structure:*
![[fullgraph.png]]
**Tr=top_root  T=Upper
Br=bot_root  B=Lower
...
function: `_cprintf()` [[#`_cprintf() `]].  `_cprintf()` adds a row of atoms that act as an internal representation of that format string. See [[#**Atoms **]], [[#Dummy Rows. Creating dummies & inserting the first atom]], and [[#Inserting more atoms]] for details atoms and on the creation and insertion of atoms.

---

#### 2) Calculating the widest atom every column:
Calling [[#`cflush()`]] initiates a tabulation and output sequence. this sequence begins with [[#`calc_max_width()`]] which traverses the structure walking down the columns starting at the `top_root` and finding which of the atoms has the largest field width. The largest field width Then gets stored in atoms in this column as `new_field_width`.

---
#### 3) Generating new field widths.
Calling [[#`cflush()`]] initiates a tabulation and output sequence. The second step in this sequence calls  [[#`generate_new_specs()`]] which uses the previouly stored values for the minimum field width and creates a new format string specification for it based an the stored flags width etc.

---

#### 4) Output

The stored data structure is then traversed by  [[#`print_something_already()`]] which will traverse down each column printing them appropriately based on their new specification (found in the last step)
...
`generate_new_specs()` generates new format strings for each atom that contain the calculated minimum width fields for each column (computed by [[#`calc_max_width()`]]) writing them to `atom->new_specification`.
...
The `cflush()` function initiates a routine that [[#`calc_max_width()` | calculates widest atom in each column]] before generating and storing new justified specifications within every atom. The newly modified structure is then printed to the specified output stream and the entire data structure is freed.
